                ...
                <mat-card-content>
                    <div class="chart-container">
                        <h3 class="graph-title card-title">Request By Status
                          <mat-icon
                              matTooltip="Distribution of SMB protocol requests grouped by command type and negotiation status."
                              [matTooltipPosition]="'above'"
                            >help_outline</mat-icon>
                        </h3>
                        <app-pie [graphOptions]="requestsByStatusCommand()"/>
                    </div>
                </mat-card-content>
            </mat-card>
        </div>
    ...
    <!-- <section>
      ...
      <div class="column-50">
        <mat-card class="radm-card">
          <mat-card-content>
            <app-area
              [graphOptions]="requestsBySharedResource()"
            />
          </mat-card-content>
        </mat-card>
      </div>
...
export class SmbTemplateComponent {
    @Input({ required: true }) requestsByStatusCommand!: Signal<PieChartInterface>;
    @Input({ required: true }) requestsByStatus!: Signal<AreaSplineChartInterface>;
    @Input({ required: true }) requestsBySharedResource!: Signal<AreaChartInterface>;
